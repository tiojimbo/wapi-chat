...
export function ConversationList({ sessionId, onSelectConversation, selectedConversationId }: ConversationListProps) {
  ...
  return (
    <aside className="w-80 border-r bg-white dark:bg-[#232326] flex flex-col h-full">
      ...
      {loading ? (
        ...
        <ul className="flex-1 overflow-y-auto">
          ...
          {conversations.map(conv => (
            <li
              key={conv.id}
              className={`flex items-center gap-3 px-4 py-3 border-b hover:bg-gray-50 dark:hover:bg-[#18181b] cursor-pointer ${selectedConversationId === conv.id ? 'bg-gray-100 dark:bg-[#18181b] font-bold' : ''}`}
